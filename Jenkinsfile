        ...
        stage('Check Background Color') {
            steps {
                script {
                    ...
                    def result = sh(script: "docker exec 6e7211c632cf grep -qiE 'background(-color)?:\\s*(red|#f00|#ff0000|rgb\\(255,\\s*0,\\s*0\\));' /usr/share/nginx/html/index.html", returnStatus: true)
